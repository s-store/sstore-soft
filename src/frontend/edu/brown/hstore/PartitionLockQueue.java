 ...
 * 
 * TODO: JOHN: Currently this extends a BlockingDeque rather than a Priority Queue.
 * This is to give us the ability to push transactions to the front of the queue
 * in the case of frontend triggers.
 * 
 */
public class PartitionLockQueue extends ThrottlingQueue<AbstractTransaction> {
     ...
     */
    @Override
    public boolean offerFirst(AbstractTransaction ts, boolean force) {
        assert(ts != null);
        assert(ts.isInitialized()) :
            String.format("Unexpected uninitialized transaction %s [partition=%d]", ts, this.partitionId);
        
        boolean retval = super.offerFirst(ts, force);
        if (debug.val)
            LOG.debug(String.format("Partition %d :: offerFirst(%s) -> %s", this.partitionId, ts, retval));
