...
public class WorkflowScheduler {
    ...
    private static AtomicBoolean allowNextTxn;
    ...
    public WorkflowScheduler(HStoreSite hstore_site) {
        ...
        this.allowNextTxn = new AtomicBoolean(true);
    }
    
    public boolean nextTxnReady()
    {
    	return allowNextTxn.compareAndSet(true, false);
    }
    
    public void allowTxns()
    {
    	allowNextTxn.set(true);
    }
    
    public boolean checkReady()
    {
    	return allowNextTxn.get();
    }
    
    
    
    /** JOHN: rewriting workflow scheduler.  Comments include original code.
...
//             (3.2) for each child with child_spid of sp, satisfy
          ...
          for(int i=0;i<children.size();i++)
          {
              ...
              if((isSPFinished(child_txn)==false))
                  return false;
