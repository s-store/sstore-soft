 ...
 * @param <E>
 */
public class ThrottlingQueue<E> implements Deque<E> {
     ...
     */
    //TODO JOHN: temporarily changing the queue datatype to a deque. We should create a separate ThrottlingDeque class.
     ...
     */
    public ThrottlingQueue(Deque<E> queue,
    ...
    /**
     * JOHN: Offer an element to the beginning of the deque. If force is true, then the element will
     * always be inserted and not count against the throttling counter.
     * @param e
     * @param force
     * @return
     */
    public boolean offerFirst(E e, boolean force) {
        boolean ret = false;
        if (force) {
            this.queue.addFirst(e);
            ret = true;
        } else if (this.throttled == false) {
            ret = this.queue.offerFirst(e);
        }
        if (ret) {
            int size = this.size.incrementAndGet();
            // If they had us force the element into the queue, then
    ...
    /**
     * JOHN: added with deque change
     */
	@Override
	public void addFirst(E e) {
		this.offerFirst(e, true);
	}
