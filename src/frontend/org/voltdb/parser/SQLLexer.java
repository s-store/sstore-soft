...
public class SQLLexer extends SQLPatternFactory
{
    ...
    private static final Pattern PAT_COPY_STATEMENT_PREAMBLE = 
	    SPF.statementLeader(
		    SPF.token("copy")
		    ).compile("PAT_SELECT_STATEMENT_PREAMBLE");
    ...
    /**
     * Check if a statement is a COPY
     * @param statement statement to check
     * @return		true if it's a COPY statement
     */
    
    public static boolean isCopy(String statement) {
	return PAT_COPY_STATEMENT_PREAMBLE.matcher(statement).matches();
    }
