...
//            if (debug.val)
    ...
    public long extractTable(Table extractTable, String destinationShim, String destinationFile, boolean caching) {
        ...
        deserializer.clear();
//        final int errorCode = nativeExtractTable(this.pointer, extractTable.getRelativeIndex(), destinationShim, destinationFile);
//        checkErrorCode(errorCode);
        final long countedRows = nativeExtractTable(this.pointer, extractTable.getRelativeIndex(), destinationShim, destinationFile, caching);
        return countedRows;
        
    }

    public long loadTableFromFile(Table loadTable, String destinationShim, String destinationFile) {
        if (debug.val) LOG.debug("Load table");
        deserializer.clear();
//        final int errorCode = nativeExtractTable(this.pointer, extractTable.getRelativeIndex(), destinationShim, destinationFile);
//        checkErrorCode(errorCode);
        final long countedRows = nativeLoadTableFromFile(this.pointer, loadTable.getRelativeIndex(), destinationShim, destinationFile);
        if (debug.val) LOG.debug("Loaded " + countedRows + " rows.");
        return countedRows;
