...
/**
 * Class that allows creation of JDBC Statement.
 * Modified for Hstore
 */
public class JDBC4Statement implements java.sql.Statement
{
    ...
    static class VoltSQL
    {
        ...
        private VoltSQL(String[] sql, int parameterCount, byte type, byte queryType, Object[] parameters)
        {
            ...
            if(parameters.length > 0) {
	            String queryStr = parameters[0].toString();	            
	            if(queryStr.contains("?")) {
	            	Object[] newpara = new Object[parameters.length - 1];
	            	for(int i = 0; i < parameters.length - 1; i++) {
	            		newpara[i] = parameters[i+1];
	            	}	   
	            	String sqlOut = replaceQuestionMarkWithValue(queryStr, newpara);
	            	this.parameterCount = 1;
	            	this.parameters = new Object[1];
	            	this.parameters[0] = sqlOut;
	            }	            
            }
        }
        
        private String replaceQuestionMarkWithValue(String stmt, Object... args){
            ...
            catch(ProcCallException e)
            {
                ...
                if (response != null) {
                    ...
                    switch (response.getStatus()) {                    
                    	...
                    	case ABORT_RESTART:
                    		throw SQLError.get(e, SQLError.GENERAL_ERROR, "RSTART", e.getMessage());
                    	case ABORT_REJECT:
                    		throw SQLError.get(e, SQLError.GENERAL_ERROR, "REJECT", e.getMessage());
                    	default:
                    		throw SQLError.get(e, SQLError.GENERAL_ERROR, String.format("status=%d", response.getStatus()), e.getMessage());
                    }
                } else {
        ...
        public static boolean hasSetAllParameters(String queryIn) {
            return queryIn.indexOf("?") == -1;
        }
    }
    ...
    {
        checkClosed();
        if(!VoltSQL.hasSetAllParameters(sql)) {
            throw SQLError.get(SQLError.PARAMETER_STILL_NULL);
        }
    ...
    {
        checkClosed();
        if (query.isQueryOfType(VoltSQL.TYPE_SELECT,VoltSQL.TYPE_EXEC, VoltSQL.TYPE_UPDATE))
        {
    ...
    @Override
    public void setQueryTimeout(int seconds) throws SQLException  // not supported
    {
        checkClosed();
        if (seconds < 0) {
