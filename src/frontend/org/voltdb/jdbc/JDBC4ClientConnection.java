...
public class JDBC4ClientConnection implements Closeable {
    private static final Logger LOG = Logger.getLogger(JDBC4ClientConnection.class);
    ...
    {			
    	ClientImpl clientTmp = (ClientImpl) ClientFactory.createClient(128,null,false,null);
...
//                System.out.println(host + " at port number " + port);
                
            	clientTmp.createConnection(null, host, port, "user","password");
                connectedAnything = true;
                if (trace.val) {
                    LOG.trace(String.format("JDBC client created connection to server %s", server));
                }
            }
            catch (UnknownHostException e) {
                LOG.error(String.format("JDBC client can not create connection to server %s", server), (debug.val ? e : null));
            }
            catch (IOException e) {
                LOG.error(String.format("JDBC client can not create connection to server %s", server), (debug.val ? e : null));
            }
            catch (RuntimeException e) {
                LOG.error(String.format("%s is not in a valid format", server), (debug.val ? e : null));
            }
        }
        ...
        if (!connectedAnything) {
            ...
            throw new IOException("JDBC client unable to connect to VoltDB cluster with servers: " + this.servers);
        }
...
//        }
    ...
    {
    	throw new UnsupportedOperationException("updateApplicationCatalog is not supported!");
    }
}
